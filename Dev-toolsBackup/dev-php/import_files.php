...
if(isset($_GET['new_file_text'])){
	echo "<div class='is_uploaded'>File wurde hochgeladen.</div>";
	$sub_name = mysqli_fetch_assoc($exist_result)['sub_name'];
	$new_file_query = "INSERT INTO `files` (`id`, `article_name`, `sub_name`, `file_name`, `file_link`, `is_test`) VALUES (NULL, '" . $article_name . "', '" . $sub_name . "', '" . $_GET['file_name'] . "', '" . mysqli_real_escape_string($conn, $_GET['new_file_text']) . "', '" . istest() . "'); ";
	mysqli_query($conn, $new_file_query);
	...
	if (isset($_GET['article_name'])){
		...
		while ($one_file = mysqli_fetch_assoc($get_files_article_result)){
			$article_files_table .= "<tr><th>" . $one_file['id'] . "</th><th>" . $one_file['article_name'] . "</th><th>" 
	...
	echo "<div class='is_deleted'>File wurde deleted.</div>";
	$delete_query = "DELETE FROM `files` WHERE `files`.`id` = " . $_GET['file_id'] . ";";

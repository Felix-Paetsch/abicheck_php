...
<?php 
	...
	$get_all_files_q = "SELECT * FROM files;";
	$get_all_files_result = mysqli_query($conn, $get_all_files_q);
	$all_files_table = "";
	$count = 0;
	while ($one_file = mysqli_fetch_assoc($get_all_files_result)){
		$all_files_table .= "<tr><th>" . $one_file['id'] . "</th><th>" . $one_file['article_name'] . "</th><th>" 
				. $one_file['sub_name'] . "</th><th>" . $one_file['file_name'] . "</th><th>" . is_test($one_file['is_test']) . "</th><th>" . htmlspecialchars($one_file['file_link']) . "</th></tr>";
		$count++;
	}

	if (isset($_GET['article_name'])){
		$get_files_article_q = "SELECT * FROM files WHERE article_name = '" . $_GET['article_name'] . "';";
		$get_files_article_result = mysqli_query($conn, $get_files_article_q);
		$article_files_table = "";
		$count = 0;
		while ($one_file = mysqli_fetch_assoc($get_files_article_result)){
			$article_files_table .= "<tr><th>" . $one_file['article_name'] . "</th><th>" 
				. $one_file['sub_name'] . "</th><th>" . $one_file['file_name'] . "</th><th>" . is_test($one_file['is_test']) . "</th><th>" . htmlspecialchars($one_file['file_link']) . "</th><th>"
				. "<form action='files.php'><input class='delete' type='submit' name='delete_file' value='Delete'>
						 <input type='hidden' name='article_name' value='" . $_GET['article_name'] . "'>
						 <input type='hidden' name='file_id' value='" . $one_file['id'] . "'></form></th></tr>";
			$count++;
		}
	}
?>
	...
	<div class="main">
		...
		<?php
			if(isset($_GET['article_name'])){
				...
				$exist_query = "SELECT sub_name FROM articles WHERE article_name = '" . $article_name . "'";
				$exist_result = mysqli_query($conn, $exist_query);
				if(mysqli_num_rows($exist_result) == 1){
					include_once "dev-php/import_files.php";
				} else {echo "<div class='ex_nicht'>Artikel '" . $article_name . "' existiert nicht.";}
			}
		?>

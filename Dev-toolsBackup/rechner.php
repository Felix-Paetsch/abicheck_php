<?php 
	...
	if (isset($_GET['new_rechner'])){
		...
		$max_query = 'SELECT MAX(rechner_id) AS "max_id" FROM rechner;';

		$max_id = mysqli_fetch_assoc(mysqli_query($conn, $max_query))['max_id'];
		header('Location: ../Dev-tools/rechner.php?rechner_id=' . $max_id);
	}
?>
...
<?php
	$rechner_query = "SELECT * FROM rechner ORDER BY rechner_id DESC";
	$rechner = mysqli_query($conn, $rechner_query);
...
<?php
	$echo_rechner = '';
	 while ($r = mysqli_fetch_assoc($rechner)) {
		...
		$echo_rechner .= '<th>'. '<form><input type="hidden" name="rechner_id" value="' . $r['rechner_id'] . '"><input type="submit" value="Update" class="unc_btn"></form>' . '</th>';
		$echo_rechner .= '</tr>';
	}
?>
		<form>
